...
import 'package:pte_mobile/screens/messaging/messaging_screen.dart';
...
class _UserProfileScreenState extends State<UserProfileScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _fetchUserDetailsAndPosts() async {
    try {
      ...
      final saved = await _postService.getUserSavedPosts(widget.userId);
      ...
      final likedFromAll = allPosts.where((post) => 
        post.likes.any((like) => like.user == widget.userId)).toList();

      List<Post> liked = likedFromAll;
      if (liked.isEmpty && totalLikes > 0) {
        try {
          liked = await _postService.getUserLikedPosts(widget.userId);
        } catch (e) {
      ...
      appBar: AppBar(
        ...
        leading: Builder(
          builder: (context) => IconButton(
            icon: Icon(Icons.arrow_back, color: Color(0xFF0632A1), size: 24),
            onPressed: () => Navigator.pop(context),
          ),
        ),
        actions: [
