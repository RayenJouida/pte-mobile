...
import 'package:pte_mobile/services/ticket_service.dart';
...
class VehicleReservationScreen extends StatefulWidget {
  ...
  const VehicleReservationScreen({
    Key? key,
...
class _VehicleReservationScreenState extends State<VehicleReservationScreen> {
  ...
  Future<void> _fetchTicketDetails() async {
    if (_formKey.currentState!.validate() && _selectedUserEmail != null) {
      ...
      try {
        ...
        final details = await _ticketService.getTicketReservation(
          _selectedUserEmail!.trim(),
          _caseIdController.text.trim(),
        );

        print('Ticket details received: $details');
        ...
        if (details['err'] == false) {
          ...
          final siteAddress = details['siteAddress'];
          ...
          if (siteAddress != null && siteAddress.toString().isNotEmpty) {
            ...
            if (destination != null) {
              ...
              QuickAlert.show(
                ...
                title: 'Success',
                text: 'Ticket details loaded successfully!',
                confirmBtnColor: const Color(0xFF0632A1),
              );
            } else {
              ...
              QuickAlert.show(
                ...
                type: QuickAlertType.warning,
                title: 'Warning',
                text: 'Could not determine location for: $siteAddress\nTicket details loaded but map location unavailable.',
                confirmBtnColor: const Color(0xFF0632A1),
              );
            }
          } else {
            ...
            QuickAlert.show(
              ...
              type: QuickAlertType.info,
              title: 'Info',
              text: 'Ticket details loaded but no site address provided.',
              confirmBtnColor: const Color(0xFF0632A1),
            );
          }
        } else {
          throw Exception(details['message'] ?? 'Failed to fetch ticket details');
        }
      } catch (e) {
        ...
        QuickAlert.show(
          ...
          type: QuickAlertType.error,
          title: 'Error',
          text: 'Failed to fetch ticket details: ${e.toString()}',
          confirmBtnColor: const Color(0xFF0632A1),
        );
      } finally {
      ...
      QuickAlert.show(
        ...
        type: QuickAlertType.warning,
        title: 'Warning',
        text: 'Please select a user and enter a valid case ID.',
        confirmBtnColor: const Color(0xFF0632A1),
      );
    }
  }
  ...
  Future<void> _createEvent() async {
    if (_formKey.currentState!.validate() && _ticketDetails != null && _selectedUserEmail != null) {
      ...
      try {
        ...
        if (_needRide == 'Yes') {
          final event = VehicleEvent(
            title: 'Vehicle Reservation - ${_ticketDetails!['ticketTitle'] ?? _caseIdController.text}',
            start: _startDateTime,
            end: _endDateTime,
            vehicleId: _selectedVehicle!.id,
            driverId: userId,
            applicantId: userId,
            destination: _ticketDetails!['siteAddress'] ?? 'Unknown',
          ...
          // Convert VehicleEvent to UserEvent for compatibility with parent widget
          ...
          final event = await _userService.createUserEvent(
            title: 'Ticket Reservation - ${_ticketDetails!['ticketTitle'] ?? _caseIdController.text}',
            start: _startDateTime!,
            end: _endDateTime!,
            engineer: userId,
            applicant: userId,
            job: _caseIdController.text.trim(),
            address: _ticketDetails!['siteAddress'] ?? 'Unknown',
            isAccepted: true,
          );
        ...
        // Fallback for type error, since event is added to DB
      ...
      QuickAlert.show(
        ...
        type: QuickAlertType.warning,
        title: 'Warning',
        text: 'Please fetch ticket details, select a user, and select valid dates.',
        confirmBtnColor: const Color(0xFF0632A1),
      );
    }
  }
                  ...
                  children: [
                    ...
                    _buildCard(
                      child: TextFormField(
                        ...
                        validator: (value) =>
                            value?.isEmpty ?? true ? 'Please enter a case ID' : null,
                      ),
                          ...
                          child: _buildCard(
                            child: TextFormField(
                              ...
                              validator: (value) => _startDateTime == null ||
                                      _startDateTime!.isBefore(DateTime.now())
                                  ? 'Please select a valid start date'
                                  : null,
                            ),
                          ).animate().fadeIn(duration: 500.ms).slideY(delay: 450.ms),
                        ),
                        SizedBox(width: 16),
                        Expanded(
                          child: _buildCard(
                          ...
                          ).animate().fadeIn(duration: 500.ms).slideY(delay: 475.ms),
                        ),
                      ],
                    ),
                    SizedBox(height: 16),
                    Container(
                      ...
                      child: ElevatedButton(
                        onPressed: _isLoading ? null : _fetchTicketDetails,
                        child: _isLoading
                            ? SizedBox(
                                height: 20,
                                width: 20,
                    ...
                    if (_ticketDetails != null && !_isLoading) ...[
                      SizedBox(height: 16),
                      _buildCard(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Row(
                              children: [
                                Text(
                                  _ticketDetails!['ticketTitle'] ?? 'N/A',
                                  style: TextStyle(
                                    fontSize: 18,
                                    fontWeight: FontWeight.bold,
                                    color: Color(0xFF0632A1),
                            ...
                            _buildReadOnlyField(
                              label: 'Client Name',
                              value: _ticketDetails!['clientName'] ?? 'N/A',
                              icon: Icons.person,
                            ),
                            SizedBox(height: 8),
                            _buildReadOnlyField(
                              label: 'Site Address',
                              value: _ticketDetails!['siteAddress'] ?? 'N/A',
                            ...
                            Row(
                              children: [
                                ...
                                SizedBox(width: 8),
                                _buildStatusIndicator(_ticketDetails!['status'] ?? 'N/A'),
                              ],
                            ),
                                    ...
                                    FlutterMap(
                                      ...
                                      children: [
                                        TileLayer(
                                          ...
                                          subdomains: ['a', 'b', 'c'],
                                          userAgentPackageName: 'com.example.pte_mobile',
                                        ),
                                      ...
                                      child: Column(
                                        children: [
                                          FloatingActionButton(
                                            ...
                                            child: Icon(Icons.add, color: Color(0xFF0632A1)),
                                            onPressed: () {
                                          ...
                                          FloatingActionButton(
                                            mini: true,
                                            backgroundColor: Colors.white,
                                            child: Icon(Icons.remove, color: Color(0xFF0632A1)),
                                            onPressed: () {
                                              final currentZoom = _mapController.zoom;
                                              _mapController.move(
                                                _mapController.center,
                      ...
                      Container(
                        ...
                        child: Column(
                          children: [
                            CircularProgressIndicator(color: Color(0xFF0632A1)),
                            SizedBox(height: 8),
                            Text('Loading ticket details...', style: TextStyle(color: Colors.grey)),
                          ],
                        ),
