...
class EngineerSidebar extends StatefulWidget {
  ...
  @override
  _EngineerSidebarState createState() => _EngineerSidebarState();
}

class _EngineerSidebarState extends State<EngineerSidebar> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    } else if (widget.currentIndex >= 2 && widget.currentIndex <= 3) {
      _isRoomsExpanded = true;
    } else if (widget.currentIndex >= 4 && widget.currentIndex <= 5) {
      _isLabsExpanded = true;
    } else if (widget.currentIndex >= 6 && widget.currentIndex <= 7) {
      _isLeaveExpanded = true;
    }
  }

  @override
  void didUpdateWidget(EngineerSidebar oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (oldWidget.currentIndex != widget.currentIndex) {
      setState(() {
        _isUsersExpanded = widget.currentIndex >= 0 && widget.currentIndex <= 1;
        _isRoomsExpanded = widget.currentIndex >= 2 && widget.currentIndex <= 3;
        _isLabsExpanded = widget.currentIndex >= 4 && widget.currentIndex <= 5;
        _isLeaveExpanded = widget.currentIndex >= 6 && widget.currentIndex <= 7;
      });
    }
  }
...
_buildSubNavItem(
  ...
  onTap: () async {
    ...
    await _navigateToPage(
      context,
      RoomReservationCalendarScreen(currentIndex: 2),
      2,
    );
  },
),                        _buildSubNavItem(
                    ...
                    _buildNavItem(
                      ...
                      children: [
                        _buildSubNavItem(
                          ...
                          onTap: () => _navigateToPage(context, const RequestLabScreen(), 4),
                        ),
                        _buildSubNavItem(
                          context,
                          icon: LineIcons.list,
                          ...
                          onTap: () => _navigateToPage(context, const RequestsByUserId(), 5),
                        ),
                      ],
                    ),
                    const SizedBox(height: 16),
                    _buildNavItem(
                      ...
                      children: [
                        _buildSubNavItem(
                          ...
                          onTap: () => _navigateToPage(context, const LeaveRequestScreen(), 6),
                        ),
                        _buildSubNavItem(
                          context,
                          icon: LineIcons.list,
                          ...
                          onTap: () => _navigateToPage(context, const MyLeaveRequestsScreen(), 7),
                        ),
                      ],
                    ),
                    const SizedBox(height: 16),
